...
type tencentCLSLogger interface {
	Name() string
	Log(msg *logger.Message) error
	Close() error
}

type newTencentCLSLoggerFunc func(*zap.Logger, *ContainerDetails) (tencentCLSLogger, error)
...
func NewDriver(zapLogger *zap.Logger) *Driver {
	driver := &Driver{
		...
		fs:               osFS{},
		newTencentCLSLogger: func(logger *zap.Logger, details *ContainerDetails) (tencentCLSLogger, error) {
			l, err := NewTencentCLSLogger(logger, details)
			if err != nil {
				return nil, err
			}
			return l, nil
...
type logStream struct {
	...
	tencentCLSLogger tencentCLSLogger
...
func (s *logStream) Close() error {
	...
	s.closedOnce.Do(func() {
		...
		if s.tencentCLSLogger != nil {
			if err := s.tencentCLSLogger.Close(); err != nil {
				errs = append(errs, fmt.Errorf("failed to close tencent cls logger: %w", err))
			}
		}
